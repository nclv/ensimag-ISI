 ...
 */
void pic_acknowledge(void) { outb(PIC_ACK_COMMAND, PIC1_COMMAND_PORT); }
...
void set_IRQ_mask(uint32_t num_IRQ) {
    uint8_t value = inb(PIC1_DATA_PORT) | (1 << num_IRQ);
    outb(value, PIC1_DATA_PORT);
}

void clear_IRQ_mask(uint32_t num_IRQ) {
    uint8_t value = inb(PIC1_DATA_PORT) & ~(1 << num_IRQ);
    outb(value, PIC1_DATA_PORT);
}
